...
Stubs CoinDCX endpoints with realistic latency and checks:
  ...
  5. config/config.yaml: parses; caps flow through resolve_assets
...
def _patched_public_coin(fake):
    orig = cb.engine.make_public_coin
    cb.engine.make_public_coin = lambda c: fake
    return orig


def test_config_caps_flow():
    cfg = cb.load_cfg(Path(__file__).resolve().parent.parent / "config" / "config.yaml")
    ...
    try:
        ...
        cb.engine.make_public_coin = orig
...
def test_bot_and_coin_drilldown():
    ...
    orig = (cb.paths.SWEEP_DIR, cb.paths.ACCOUNTS_DIR, cb.paths.ACCOUNT_CSV,
            cb.paths.SIGNATURE_FILE)
    cb.paths.SWEEP_DIR = tmp
    cb.paths.ACCOUNTS_DIR = tmp / "accounts"
    cb.paths.ACCOUNT_CSV = tmp / "accounts.csv"
    cb.paths.SIGNATURE_FILE = tmp / "grid_signature.txt"
    try:
        rows = [
        ...
        a1 = cb.paths.ACCOUNTS_DIR / "acc_001"
        ...
        a2 = cb.paths.ACCOUNTS_DIR / "acc_002"
        ...
        (cb.paths.SWEEP_DIR, cb.paths.ACCOUNTS_DIR, cb.paths.ACCOUNT_CSV,
         cb.paths.SIGNATURE_FILE) = orig
